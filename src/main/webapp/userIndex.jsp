		...
		<div class="container">
			...
			<div class="row">
				<div class="col-sm-2 col-xs-5" style="padding-left: 0;padding-right: 0">
					<img src="/upload/<sec:authentication property="principal.username"/>.png" class="img-responsive" alt="Image" id="mylogo" onerror="this.src='img/img_no_find.png'">
				...
				<!-- 	<input type="button" id="uploadButton" value="上传" class="btn" /> -->
					...
					<a style="font-size: 10px;" href="loginwuziqi">进入五子棋</a>
					<a style="font-size: 10px;" href="loginxiangqi">进入象棋</a>
					<br/>
					<a href="#msglist">我的消息</a>&nbsp;
					<a href="#friendul">我的好友</a>
				</div>
				<div class="col-sm-10 col-xs-7">
					<h2 class="text-right" style="border-bottom: 2px solid black" id="user_userid">${user.userid }</h2>
					<h2 class="text-right" id="user_win_fail">win:${user.win} &nbsp;lose:${user.fail } </h2>
				</div>
			</div>
		</div>
...
<script>
 $('#fileupload').fileupload({
        dataType: 'json',
            ...
            $("#mylogo").attr({
				"src": "/upload/" +<sec:authentication property="principal.username"/>  + ".png?id="+d.getTime()
			})
